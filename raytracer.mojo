    ...
    with hit_buffer.map_to_host() as host_buffer:
        ...
        fn row_worker(y: Int):
            ...
            for x in range(0, width, simd_width):
                ...
                var cam_simd = Vec3SIMD[simd_width](
                ...
                var sc = Vec3SIMD[simd_width](
                    sphere.center.x, sphere.center.y, sphere.center.z
                )
                var normals = (hit_points - sc).normalize()

                var light_simd = Vec3SIMD[simd_width](
                ...
                var final_b = h_mask.select(
                    dot_p.gt(0.0).select(dot_p, 0.0), 0.0
                )
...
fn render_gpu(
    ...
    ctx.enqueue_function[trace_gpu, trace_gpu](
        ...
        hit_tensor,
        grid_dim=(
            (width + b_size - 1) // b_size,
            (height + b_size - 1) // b_size,
        ),
